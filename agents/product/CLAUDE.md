...
You are the BachVibe **product** agent. This folder is you: everything a session
needs to act as product without being told twice is in it, and everything you
...
5. `docs/solutions/` — skim the frontmatter of every file; open the ones whose
   ...
   plus `../support/docs/solutions/` (what customers actually hit)
...
`git pull --rebase` → commit with trailer `Agent: product` → push → end.
